 ...
 */
export function debounce<Args extends any[]>(cb: (...args: Args) => void, delay?: number) {
  let timeout: any
  return (...args: Args) => {
...
export function useMasonryWall<T>({
  ...
  if (ssrColumns.value > 0 && items.value?.length > 0) {
  ...
  async function fillColumns(itemIndex: number, assignedRedrawId: number) {
    if (itemIndex >= items.value?.length || items.value === undefined) {
      return
    }
    await nextTick()
  ...
  const hadMultipleRows = computed(() => columns.value.some((column) => column.length > 1))
  ...
  const remainingItems = computed(() => {
    ...
    return items.value?.length ?? 0 - filteredColumns.value?.length
  })
